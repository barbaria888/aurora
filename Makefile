...
prod-prebuilt:
	...
	@echo "  - Frontend: $$(grep -E '^FRONTEND_URL=' .env | cut -d= -f2- | tr -d '\"' || echo 'http://localhost:3000')"
	@echo "  - Backend API: $$(grep -E '^NEXT_PUBLIC_BACKEND_URL=' .env | cut -d= -f2- | tr -d '\"' || echo 'http://localhost:5080')"
	@echo "  - Chatbot WebSocket: $$(grep -E '^NEXT_PUBLIC_WEBSOCKET_URL=' .env | cut -d= -f2- | tr -d '\"' || echo 'ws://localhost:5006')"
	@echo "  - Vault UI: http://$$(grep -E '^FRONTEND_URL=' .env | cut -d= -f2- | tr -d '\"' | sed 's|.*://||;s|:.*||' || echo 'localhost'):8200"
	@echo ""
